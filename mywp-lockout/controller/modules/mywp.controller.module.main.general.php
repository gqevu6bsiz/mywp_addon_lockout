...
if ( ! class_exists( 'MywpControllerModuleLockoutMainGeneral' ) ) :

final class MywpControllerModuleLockoutMainGeneral extends MywpControllerAbstractModule {

  static protected $id = 'lockout_main_general';
  ...
  private static function is_current_plugin( $plugin_file_name = false ) {
    ...
    if ( strpos( MYWP_LOCKOUT_PLUGIN_BASENAME , $plugin_file_name ) === false ) {
  ...
  public static function load_plugins() {
    ...
    if( is_multisite() ) {
      ...
      add_filter( 'network_admin_plugin_action_links_' . MYWP_LOCKOUT_PLUGIN_BASENAME , array( __CLASS__ , 'plugin_action_links' ) , 10 , 4 );

    } else {

      add_filter( 'plugin_action_links_' . MYWP_LOCKOUT_PLUGIN_BASENAME , array( __CLASS__ , 'plugin_action_links' ) , 10 , 4 );
  ...
  public static function plugin_row_meta( $plugin_meta , $plugin_file , $plugin_data , $status ) {
    ...
    if( ! empty( $plugin_info['document_url'] ) ) {
      ...
      $plugin_meta[] =  sprintf( '<a href="%1$s" target="_blank">%2$s</a>' , esc_url( $plugin_info['document_url'] ) , __( 'Document' , 'mywp-lockout' ) );

    }

    $plugin_meta = apply_filters( 'mywp_lockout_plugin_row_meta' , $plugin_meta , $plugin_file , $plugin_data , $status );
  ...
  public static function plugin_action_links( $actions , $plugin_file , $plugin_data , $context ) {
    ...
    $actions = apply_filters( 'mywp_lockout_plugin_action_links' , $actions , $plugin_file , $plugin_data , $context );
  ...
  public static function in_plugin_update_message( $plugin_data , $response ) {
    ...
    printf( __( 'There is a new version of %1$s available. <a href="%2$s" %3$s>View version %4$s details</a>.' ) , $response->new_version , $plugin_info['github'] , '' ,  MYWP_LOCKOUT_NAME );
  ...
  public static function admin_print_footer_scripts() {
    ...
    printf( 'tr#%s-update .update-message p { display: none; }' , MYWP_LOCKOUT_PLUGIN_DIRNAME );
    printf( 'tr#%s-update .update-message p.show { display: block; }' , MYWP_LOCKOUT_PLUGIN_DIRNAME );
...
MywpControllerModuleLockoutMainGeneral::init();
