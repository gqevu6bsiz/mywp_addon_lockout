...
if ( ! class_exists( 'MywpControllerModuleLockoutUpdater' ) ) :

final class MywpControllerModuleLockoutUpdater extends MywpControllerAbstractModule {

  static protected $id = 'lockout_updater';

  static private $schedule_hook = 'mywp_lockout_version_check';
  ...
  public static function get_remote() {
    ...
    if( empty( $remote_result ) ) {
      ...
      $error->add( 'not_results' , __( 'Connection lost or the server is busy. Please try again later.' , 'mywp-lockout' ) );
    ...
    if( $remote_code !== 200 ) {
      ...
      if( ! empty( $remote_body ) ) {
        ...
        if( ! empty( $maybe_json ) && ! empty( $maybe_json->message ) ) {
          ...
          $error->add( 'invalid_json' , sprintf( '[%d] %s' , $remote_code , __( 'An error has occurred. Please reload the page and try again.' , 'mywp-lockout' ) ) );

        }

      } else {

        $error->add( 'invalid_connection' , sprintf( '[%d] %s' , $remote_code , __( 'Connection lost or the server is busy. Please try again later.' , 'mywp-lockout' ) ) );
    ...
    if( empty( $remote_body ) ) {
      ...
      $error->add( 'invalid_remote_body' , __( 'An error has occurred. Please reload the page and try again.' , 'mywp-lockout' ) );
  ...
  private static function get_remote_download_link() {
    ...
    $download_file_path = sprintf( '%s.%s.zip' , MYWP_LOCKOUT_PLUGIN_DIRNAME , self::get_latest() );
  ...
  public static function get_latest() {
    ...
    if( ! is_object( $maybe_remote_json ) or ! isset( $maybe_remote_json->tag_name ) ) {
      ...
      $error->add( 'invalid_remote_json' , __( 'Invalid remote Json data. Please try again.' , 'mywp-lockout' ) );
  ...
  public static function is_latest() {
    ...
    $latest_compare = version_compare( $latest , MYWP_LOCKOUT_VERSION , '<=' );
  ...
  public static function plugins_api( $false , $action , $args ) {
    ...
    if( $args->slug !== MYWP_LOCKOUT_PLUGIN_DIRNAME ) {
    ...
    $plugin_api = array(
      'name' => MYWP_LOCKOUT_NAME,
      'slug' => MYWP_LOCKOUT_PLUGIN_BASENAME,
...
MywpControllerModuleLockoutUpdater::init();
